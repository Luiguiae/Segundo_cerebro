...
- **Buscador de correlaciones + Graduador de borradores** (mejora-011, 2026-08-24): "Jarvis, busca correlaciones" identifica conceptos sub-conectados y propone correlaciones nuevas (con autocrítica adversarial anti-truismo antes de proponer); "Jarvis, gradúa los borradores" corre profundización dirigida al hueco de rúbrica específico de cada `borrador`. Ninguno de los dos escribe directo al vault — ambos proponen en `Inbox/*.tmp.md`; "Jarvis, revisa propuestas pendientes" cierra el loop con aprobación explícita. Respaldo: rutina cloud semanal (`vault-correlaciones-y-graduacion-semanal`, lunes 06:00 Lima) que corre ambos comandos y hace push de las propuestas a `Inbox/` si encuentra algo — ver `docs/plan-011.md`.
